...
clear
close all
load data_02-May-2016.mat
intensG3 = []; intensR3 = [];
for f = 1
[ncells, nframes] = size(intensGAll{f});

intensG = intensGAll{f};
intensR = intensRAll{f};

avgintensR = mean(intensR,1);
avgintensG = mean(intensG,1);
[bgR, pbleachR] = rem_pbleach(avgintensR,40);
[bgG, pbleachG] = rem_pbleach(avgintensG,50);

[ncells,nframes] = size(intensG);
time = (1/5)*(1:nframes);

intensR2 = bsxfun(@rdivide,intensR,pbleachR');
intensG2 = bsxfun(@rdivide,intensG,pbleachG');
for j = 1:ncells
    tmp = intensG2(j,:);
    [thr,sorh,keepapp] = ddencmp('den','wv',tmp);
    intensDeNoiseG(j,:) = wdencmp('gbl',tmp,'db3',2,thr,sorh,keepapp);
    tmp = intensR2(j,:);
    [thr,sorh,keepapp] = ddencmp('den','wv',tmp);
    intensDeNoiseR(j,:) = wdencmp('gbl',tmp,'db3',2,thr,sorh,keepapp);
end
[tOnG,tOffG,nBlinksG] = find_Ca_blinks(intensG2);
[tOnR,tOffR,nBlinksR] = find_PROPS_blinks(intensR2);

blinkers = find(~cellfun(@isempty,tOnG));
nBlinkers = length(blinkers);


intensG3 = [intensG3; intensG2(blinkers,:)];
intensR3 = [intensR3; intensR2(blinkers,:)];
% intensG3 = [intensG3; intensDeNoiseG(blinkers,:)];
% intensR3 = [intensR3; intensDeNoiseR(blinkers,:)];
end
[ncellsTot,~] = size(intensG3);

X(1,1:nframes,1:ncellsTot) = intensR3';
X(2,1:nframes,1:ncellsTot) = intensG3';

%% Parameters

ntrials   = ncellsTot;     % number of trials
nobs      = nframes;   % number of observations per trial
...
acmaxlags = 20000;   % maximum autocovariance lags (empty for automatic calculation)
...
figure
plot(lam(150:end),squeeze(f(2,1,150:end)))
hold all
plot(lam(150:end),squeeze(f(1,2,(150:end))))
hold off
xlabel('Frequency (Hz)')
ylabel('GC')
legend('V -> Ca','Ca -> V')
saveas(gca,[saveDir filesep 'Grainger_causality.png'])
saveas(gca,[saveDir filesep 'Grainger_causality.fig'])

...
void Calc_Dimension(double** data, size_t dim, size_t max_dim, size_t num, char TAKENS,char CORR,char SPARSE)
{
  //long i,j,k,counter;
  long i,j,counter;
  size_t l,m,k;
  ...
  double dist;
  double* min_dist=new double[num];
  for(i=0;i<num;i++)
   min_dist[i]=1E10;
  ...
  for(m=1;m<=max_dim;m++)
  {
    ...
    if(fabs(slope_est[m-1])<minimum) { minimum=slope_est[m-1]; DIM= double(m); }
